...
% simplify and built the new code by gwt on December 15 2015
close all; clear; clc;
...
nit    = 500;   % number of ICP iterations for each scan-match
gate1  = 2.5; % 1st data association gate for each point in scan
gate2  = 0.2;% 2nd data association gate for each point in scan
...
for i = 1 : len
    %% Run ICP
    relpos_ba    = icp(laser(i,:), laser(i+1,:),relpos_ba, gate2, nit);
    %disp(relpos_ba)
    relpos_ab    = inverse(relpos_ba);
    
    %%

    pos = compound(pos,relpos_ab)
  
    path(:, i+1) = pos;
    robPos       = compound(pos, rob);
    points       = get_laser_points(laser(i+1, :), maxR);
